var app = Vue.createApp({
  ...
  mounted() {
              ...
              switch (newWorker.state) {
                case 'installed':
                  if (navigator.serviceWorker.controller) {
                    setTimeout(() => {document.querySelector('#reload-modal').classList.add('shown')}, 1000);
                  }
                  break;
              }
            })
    ...
    const observer = new MutationObserver(() => {
      this.$root.isModalVisible = document.querySelector('.modal.shown') !== null;
    })
...
app.component('route-modal', {
  ...
  computed: {
    busIconSrc() { return `/images/icons/${this.busColor.toLowerCase()}-bus-icon.png`},
  },
  methods: {
    changeDirection() {
      ...
      if (this.mapEnabled && this.busMap && this.busMap.getLayer('route')) {
    ...
    disableMap() {
      ...
      setTimeout(() => {
        ...
        var titleHeight = document.querySelector('.route-bus-title').offsetHeight
    ...
    requestRoute(route,color) {
      ...
      this.$parent.intervals = [dataInterval, trafficInterval];
    },
    resetMap() {
    ...
    scrollToWarning() {
      ...
      mainRouteInfo.scroll({top: (this.busMap && this.mapEnabled) ? suspendedParent.offsetTop - document.querySelector('.route-navbar').offsetHeight : suspendedParent.offsetTop + document.querySelector('.route-bus-title').offsetHeight, behavior: 'smooth'});
      ...
      if (this.mapEnabled) {
        this.waitUntil(() => {
          ...
          for (let bus of this.busInfoLocations.slice()) {
            ...
            if (this.focusingStation && !busElement.id.includes(this.arrivingBuses[this.currentlyOpenedIndex][0].plate)) {
              busElement.style.setProperty('display', 'none', 'important');
            } else {
              busElement.style.removeProperty('display');
            }
          ...
          for (let i = 0; i < this.busRouteTraffic.length-1; i++) {
            if (typeof(this.busRouteTraffic[i].routeCoordinates) === 'string') {
              ...
              if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
                ...
                else var color = '#999999'
              }
              routeCoordinates.pop();
              let opacity = 1;
              let index = this.currentlyOpenedIndex;
              if (index != null && this.focusingStation) {
                let closestStationIndex = this.arrivingBuses[index][0].currentStation - 1;
                if (i >= index || i < closestStationIndex) {
                  opacity = 0.25;
                }
